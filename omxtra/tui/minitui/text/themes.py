...
Color values are copied from the resolved dump at `omdev/tui/rich/textual/dark.py` (Textual's `$variable` references
and alpha blending pre-computed against its `#121212` page background) - minitui does not import omdev. Colors are
authored as truecolor and downgrade automatically on lesser terminals via `colors.downgrade_color`; surfaces pick the
depth from the environment (`colors.detect_color_depth`).
