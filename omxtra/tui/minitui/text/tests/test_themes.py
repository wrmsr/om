...
LIBRARY_TAGS = (
    *(f'md.h{i}' for i in range(1, 7)),
    'md.bold', 'md.italic', 'md.strike',
    'md.code', 'md.code.inline',
    'md.quote', 'md.quote.marker',
    'md.list.marker', 'md.link', 'md.link.url', 'md.rule',

    'code.keyword', 'code.builtin', 'code.def', 'code.string', 'code.comment', 'code.number',
    'code.decorator', 'code.type',
    'code.diff.add', 'code.diff.del', 'code.diff.hunk', 'code.diff.meta',

    'card.expander', 'card.summary', 'card.summary.dim', 'card.detail', 'card.allow', 'card.deny',
    *(f'card.glyph.{state.name.lower()}' for state in CardState),

    'popup.label', 'popup.desc', 'popup.selected', 'popup.selected.desc',

    'vim.selection', 'vim.cursor', 'vim.search.match', 'vim.search.current',
)
