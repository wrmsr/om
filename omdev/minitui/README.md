...
- **text/** - adapts the target-neutral `omcore.text.styled` model into terminal segments: the `Theme` facade, the
  segment/styled-text conversions, segment wrapping over the core grid lowering, and streaming markdown with swappable
  backends (zero-dep internal / omcore pdcmark / markdown-it) over a shared block model. Terminal colors, depth
  downgrade, SGR emit/parse, and headless ANSI rendering of `StyledText` and `StyledDocument` live in
