...
class MysqlPacket:
    ...
    def get_bytes(self, position, length=1):
        ...
        Position is start of payload (first four packet header bytes are not included) starting at index '0'.

        No error checking is done.  If requesting outside end of buffer an empty string (or string shorter than
        'length') may be returned!
        """
    ...
    def read_length_encoded_integer(self):
        ...
        Length coded numbers can be anywhere from 1 to 9 bytes depending on the value of the first byte.
        """
    ...
    def read_length_coded_string(self):
        ...
        A 'Length Coded String' consists first of a length coded (unsigned, positive) integer represented in 1-9 bytes
        followed by that many bytes of binary data.  (For example "cat" would be "3cat".)
        """
...
class FieldDescriptorPacket(MysqlPacket):
    ...
    Parsing is automatically done and the results are exported via public attributes on the class such as: db,
    table_name, name, length, type_code.
    """
    ...
    def _parse_field_descriptor(self, encoding):
        ...
        # 'default' is a length coded binary and is still in the buffer? not used for normal result sets...
    ...
    def __str__(self):
        return (
            f'{self.__class__} '
            f'{self.db!r}.{self.table_name!r}.{self.name!r}, '
            f'type={self.type_code}, '
...
class OKPacketWrapper:
    """
    OK Packet Wrapper. It uses an existing packet object, and wraps around it, exposing useful variables while still
    providing access to the original packet objects variables and methods.
    """
...
class EOFPacketWrapper:
    """
    EOF Packet Wrapper. It uses an existing packet object, and wraps around it, exposing useful variables while still
    providing access to the original packet objects variables and methods.
    """
...
class LoadLocalPacketWrapper:
    """
    Load Local Packet Wrapper. It uses an existing packet object, and wraps around it, exposing useful variables while
    still providing access to the original packet objects variables and methods.
    """
