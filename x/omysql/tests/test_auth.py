...
# omysql.connections.DEBUG = True
# omysql._auth.DEBUG = True
...
def test_sha256_no_password():
    con = omysql.connect(user="nopass_sha256", host=host, port=port, ssl=None)
    con.close()


def test_sha256_no_passowrd_ssl():
    con = omysql.connect(user="nopass_sha256", host=host, port=port, ssl=ssl)
    con.close()
...
def test_caching_sha2_no_password():
    con = omysql.connect(user="nopass_caching_sha2", host=host, port=port, ssl=None)
    con.close()


def test_caching_sha2_no_password_ssl():
    con = omysql.connect(user="nopass_caching_sha2", host=host, port=port, ssl=ssl)
    con.close()
...
def test_caching_sha2_password():
    ...
    omysql.connections._DEFAULT_AUTH_PLUGIN = "mysql_native_password"
    con = omysql.connect(
        user="user_caching_sha2",
        password=pass_caching_sha2,
        host=host,
    ...
    con.query("FLUSH PRIVILEGES")
    con.close()
    omysql.connections._DEFAULT_AUTH_PLUGIN = None
