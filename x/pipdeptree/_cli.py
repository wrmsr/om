...
def parse_packages(value: str | None) -> tuple[list[str], dict[str, set[str]]]:
    ...
    An entry like ``foo[bar,baz]`` yields the name pattern ``foo`` and the extras ``{bar, baz}``; plain entries carry no
    extras. The extras are matched against installed package names later, so wildcard patterns such as ``foo*[bar]``
    apply to every package matching ``foo*``.
    """
