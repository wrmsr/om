...
def get_installed_distributions(
    ...
    should_query_interpreter = (
        not supplied_paths and
        (pathlib.Path(interpreter).absolute() != pathlib.Path(sys.executable).absolute())
    )
...
def filter_valid_distributions(
        iterable_dists: ta.Iterable[importlib.metadata.Distribution],
) -> list[importlib.metadata.Distribution]:
    ...
    # take it one step further and warn the user that there are duplicate packages in their environment. See
    # https://github.com/pypa/pip/blob/7c49d06ea4be4635561f16a524e3842817d1169a/src/pip/_internal/metadata/importlib/_envs.py#L34
    seen_dists: dict[str, importlib.metadata.Distribution] = {}
    first_seen_to_already_seen_dists_dict: dict[importlib.metadata.Distribution, list[importlib.metadata.Distribution]] = {}  # noqa
