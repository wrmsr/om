...
class Signaling(lang.Abstract):
    """
    Wakeups only, never transport: a notification can be dropped, so the loop polls on wake *and* on a timer. `notify`
    is called inside the send transaction so it fires on commit and never for a rolled-back batch.
    """
