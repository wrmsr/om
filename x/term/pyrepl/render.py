...
class RenderCell:
    ...
    text: str
    width: int
    style: StyleRef = dc.field(default_factory=StyleRef)
    controls: tuple[str, ...] = ()
...
class RenderLine:
    ...
    ``text`` is the pre-rendered terminal string (characters + SGR escapes); ``width`` is the total visible column
    count.
    """
    ...
    @classmethod
    def from_cells(cls, cells: ta.Iterable[RenderCell]) -> ta.Self:
    ...
    def from_parts(
        ...
        styles: ta.Sequence[RenderStyle] | None = None,
    ) -> ta.Self:
        if styles is None:
    ...
    @classmethod
    def from_rendered_text(cls, text: str) -> ta.Self:
...
class ScreenOverlay:
    ...
    If *insert* is True, lines are spliced in (shifting content down); if False (default), lines replace existing
    content at *y*.

    Overlays are used to display tab completion menus and status messages. For example, a tab-completion menu inserted
    below the input::
...
class RenderedScreen:
    ...
    ``lines`` holds the base content; ``composed_lines`` is the final result after overlays (completion menus, messages)
    are applied::
...
class LineDiff:
    ...
    When the user types ``e`` so the row changes from ``>>> nam`` to ``>>> name``::
...
def render_cells(
    cells: ta.Sequence[RenderCell],
    visual_style: str | None = None,
) -> str:
    """
    Render a sequence of cells into a terminal string with SGR escapes.

    Tracks the active SGR state to emit resets only when the style actually changes, minimizing output bytes.

    If *visual_style* is given (used by redraw visualization), it is appended to every cell's style.
    """
...
def diff_render_lines(old: RenderLine, new: RenderLine) -> LineDiff | None:
    ...
    while prefix < max_prefix and old.cells[prefix] == new.cells[prefix]:
        # Stop at any cell with non-SGR controls, since those might affect cursor position and must be re-emitted.
