...
class WrappedRow:
    ...
    When a line overflows the terminal width, it splits into multiple rows with a ``\\`` continuation marker::
...
def layout_content_lines(
    ...
    A short line passes through as one ``WrappedRow``; a long line is split at the column boundary with ``\\`` markers::
