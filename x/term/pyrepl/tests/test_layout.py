...
class TestLayoutMap:
    def test_empty(self):
        lm = LayoutMap.empty()
        assert len(lm.rows) == 1
        assert lm.max_row() == 0
        assert lm.max_column(0) == 0
    ...
    def test_pos_to_xy_empty_rows(self):
        lm = LayoutMap(())
        assert lm.pos_to_xy(0) == (0, 0)

    def test_pos_to_xy_single_row(self):
        lm = LayoutMap((LayoutRow(4, (1, 1, 1), buffer_advance=3),))
        assert lm.pos_to_xy(0) == (4, 0)
        assert lm.pos_to_xy(1) == (5, 0)
        assert lm.pos_to_xy(2) == (6, 0)
        assert lm.pos_to_xy(3) == (7, 0)
    ...
    def test_pos_to_xy_with_suffix(self):
        ...
        assert lm.pos_to_xy(3) == (1, 1)

    def test_xy_to_pos_empty_rows(self):
        lm = LayoutMap(())
        assert lm.xy_to_pos(0, 0) == 0

    def test_xy_to_pos_single_row(self):
        lm = LayoutMap((LayoutRow(4, (1, 1, 1), buffer_advance=3),))
        assert lm.xy_to_pos(4, 0) == 0
        assert lm.xy_to_pos(5, 0) == 1
        assert lm.xy_to_pos(6, 0) == 2
        assert lm.xy_to_pos(7, 0) == 3
    ...
    def test_xy_to_pos_multi_row(self):
        ...
        assert lm.xy_to_pos(4, 0) == 0
        assert lm.xy_to_pos(4, 1) == 4
        assert lm.xy_to_pos(5, 1) == 5
    ...
    def test_xy_to_pos_trailing_all_zero_width(self):
        ...
        assert lm.xy_to_pos(1, 0) == 2


class TestLayoutContentLines:
    def test_zero_width_returns_empty(self):
        result = layout_content_lines((), 0, 0)
        assert result.wrapped_rows == ()
        assert result.layout_map.rows == ()
    ...
    def test_single_short_line(self):
        ...
        assert len(result.wrapped_rows) == 1
        row = result.wrapped_rows[0]
        assert row.prompt_text == '>>> '
        assert row.prompt_width == 4
        assert row.suffix == ''
        assert row.buffer_advance == 4  # 3 chars + newline
    ...
    def test_single_line_no_newline(self):
        ...
        assert len(result.wrapped_rows) == 1
        assert result.wrapped_rows[0].buffer_advance == 2
    ...
    def test_empty_body(self):
        ...
        assert len(result.wrapped_rows) == 1
        assert result.wrapped_rows[0].buffer_advance == 1  # just newline
    ...
    def test_line_wraps(self):
        ...
        assert len(result.wrapped_rows) == 2
        first, second = result.wrapped_rows
        assert first.prompt_text == '>>> '
        assert first.suffix == '\\'
        assert first.suffix_width == 1
        # First row: 10 - 4(prompt) - 1(suffix) = 5 chars
        assert first.buffer_advance == 5
        # Second row: continuation with no prompt
        assert second.prompt_text == ''
        assert second.prompt_width == 0
        assert second.buffer_advance == 4  # remaining 3 + newline
        assert second.suffix == ''
    ...
    def test_wrapping_forces_progress(self):
        ...
        # width=2 means prompt(1) + char(1) = 2, which fits (< width would be false for width=2 since 1+1 >= 2), so it
        # wraps but forces progress
        result = layout_content_lines((line,), 2, 0)

        assert len(result.wrapped_rows) == 1
        assert result.wrapped_rows[0].buffer_advance == 1

    def test_layout_map_matches_wrapped_rows(self):
        line = _content_line('abc')
        result = layout_content_lines((line,), 80, 0)

        assert len(result.layout_map.rows) == len(result.wrapped_rows)
        assert result.layout_map.rows[0].prompt_width == 4
        assert result.layout_map.rows[0].char_widths == (1, 1, 1)
    ...
    def test_multiple_lines(self):
        ...
        assert len(result.wrapped_rows) == 2
        assert result.wrapped_rows[0].buffer_advance == 4  # abc + \n
        assert result.wrapped_rows[1].buffer_advance == 3  # de + \n
    ...
    def test_leading_prompt_lines(self):
        ...
        assert result.wrapped_rows[0].fragments == leading
        # Body row has prompt and content
        assert result.wrapped_rows[1].prompt_text == '>>> '
    ...
    def test_wrapped_line_layout_rows_have_suffix(self):
        ...
        assert first_layout.suffix_width == 1
        second_layout = result.layout_map.rows[1]
        assert second_layout.suffix_width == 0
    ...
    def test_pos_to_xy_through_layout(self):
        ...
        assert lm.pos_to_xy(0) == (4, 0)  # after prompt
        assert lm.pos_to_xy(1) == (5, 0)
        assert lm.pos_to_xy(2) == (6, 0)
        assert lm.pos_to_xy(3) == (7, 0)  # end of line
