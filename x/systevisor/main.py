...
def _systevisor_main_serve(args: argparse.Namespace) -> int:
    ...
    controller = injector.provide(SystevisorConfigController)
    control = injector.provide(SystevisorControlService)
    control_plane = injector.provide(SystevisorControlPlane)
    poller = injector.provide(FdioPoller)
    codec = injector.provide(SystevisorJsonCodec)
    try:
        result = controller.reload(initial=True)
        if not result.attempt.applied or result.snapshot is None:
            _systevisor_main_print_json(result.attempt, codec, 2)
            return 2
...
def _systevisor_main_run(args: argparse.Namespace) -> int:
    ...
    coordinator.engine.state.startup_collection = collection_name
    try:
        result = controller.reload(initial=True)
        if not result.attempt.applied or result.snapshot is None:
            _systevisor_main_print_json(result.attempt, codec, 2)
...
def _systevisor_main_parser() -> argparse.ArgumentParser:
    ...
    for command in ('status', 'units', 'collections', 'config', 'operations', 'reload', 'check', 'shutdown'):
