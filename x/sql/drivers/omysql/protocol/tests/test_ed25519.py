...
def _reference_ed25519_password(password: bytes, scramble: bytes) -> bytes:
    ...
    signature_scalar = bindings.crypto_core_ed25519_scalar_add(
        bindings.crypto_core_ed25519_scalar_mul(k, s),
        r,
    )
...
def test_base_multiplication_matches_libsodium(scalar: int) -> None:
    ...
    expected = bindings.crypto_scalarmult_ed25519_base_noclamp(
...
def test_result_is_a_standard_verifiable_ed25519_signature(
    ...
    Ed25519PublicKey.from_public_bytes(public_bytes).verify(signature, scramble)
...
def test_auto_falls_through_unavailable_backends(
    ...
    def unavailable(name: str):
        def run(password: bytes, scramble: bytes) -> bytes:
            calls.append(name)
            raise sut.BackendUnavailable(name)
        return run
...
def test_explicit_backend_does_not_fall_through(
    ...
    def unavailable(password: bytes, scramble: bytes) -> bytes:
        raise sut.BackendUnavailable('deliberate')
    ...
    with pytest.raises(sut.BackendUnavailable, match='deliberate'):
