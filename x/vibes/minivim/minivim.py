...
Cross-cutting, exactly like vim:
  * `.` repeat is a *keystroke recorder + replayer* (vim's "redo buffer"),
    not a semantic diff -- this is why `ciwfoo<Esc>` repeats with new text.
  * curswant: j/k remember the column you *want*, `$` pins it to end-of-line.
  * Many "commands" are synonyms compiled to op+motion: x=dl, D=d$, C=c$,
    s=cl, S=cc, Y=yy.  (vim does the same internally.)
  * Registers carry a kind; linewise `p` opens lines, charwise `p` splices.

The engine talks to documents only through the `Buffer` protocol (5 methods),
so adapting a rope / piece table / GUI text store means implementing those
five methods and pumping decoded keys into `Engine.feed()`.  See demo_tty.py
for a ~60-line curses frontend.

Deliberately out of scope (noted where relevant): ex commands (`:`), search
(`/ ? n N`), marks/jumplist, macros (`q`), blockwise visual, undo *tree*
(we keep a linear snapshot stack), multibyte/virtual columns.
"""
...
class Kind(Enum):
    ...
    EXCLUSIVE = auto()   # charwise, target char NOT covered (w, b, h, 0, F, T)
    INCLUSIVE = auto()   # charwise, target char covered      (e, f, t, $)
    LINEWISE = auto()    # whole lines                        (j, k, G, gg, dd)
...
def _retreat(buf: Buffer, p: Pos) -> Optional[Pos]:
    if p.col > 0:
        return Pos(p.row, p.col - 1)
    if p.row > 0:
...
def _cls(ch: Optional[str], big: bool) -> int:
...
def find_char(buf: Buffer, p: Pos, ch: str, count: int, forward: bool,
              till: bool, repeat: bool = False) -> Optional[Pos]:
    """`f t F T` (current line only, like vim).  `repeat` handles the classic
    `;`-after-`t` stickiness: skip a target we're already sitting against."""
    line = buf.get_line(p.row)
    col = p.col
    if repeat and till:
...
# ──────────────────────────────────────────────────────────────────────────
# Resolving (start, motion) into an operable Span.
# Charwise spans use [start, end) with end.col allowed to equal len(line)
# ("through end of line, not the newline").  Linewise spans are row ranges.
# ──────────────────────────────────────────────────────────────────────────
...
def resolve(buf: Buffer, start: Pos, mr: MotionResult) -> Optional[Span]:
    ...
    if mr.kind is Kind.INCLUSIVE:
        ...
        # vim's two `:help exclusive` adjustments.  Together they produce the
        # behavior everyone knows without w/b special-casing anything:
...
# ──────────────────────────────────────────────────────────────────────────
# Registers.  Contents are stored as a list of line-pieces plus a kind;
# the kind decides whether `p` opens new lines or splices into the current.
# ──────────────────────────────────────────────────────────────────────────
...
class Registers:
    ...
    def get(self, name: str) -> Optional[RegValue]:
...
# ──────────────────────────────────────────────────────────────────────────
# Text objects.  Unlike motions they yield a Span directly -- there is no
# "start position + direction", just "the thing under the cursor".
# ──────────────────────────────────────────────────────────────────────────
...
def textobj(buf: Buffer, p: Pos, around: bool, obj: str, count: int) -> Optional[Span]:
    if obj in "wW":
...
def _obj_pair(buf, p, around, open_ch, close_ch):
    # Backward for the unmatched open (cursor sitting ON open matches itself),
    # forward for the unmatched close.  Multi-line.
    ...
    # vim promotes the *inner* object to linewise when the open bracket ends
    # its line and only whitespace precedes the close bracket on its line --
    # this is why `di{` on a code block keeps the braces on their own lines.
    if (open_pos.col == _llen(buf, open_pos.row) - 1
            and close_pos.col <= _first_nonblank(buf, close_pos.row)
            and close_pos.row > open_pos.row + 1):
        return Span(Kind.LINEWISE, Pos(open_pos.row + 1, 0),
                    Pos(close_pos.row - 1, 0))
    inner = _advance(buf, open_pos)
    return Span(Kind.EXCLUSIVE, inner, close_pos)


def _obj_quote(buf, p, around, q):
    # Current line only, like vim.  Pair quotes left-to-right; take the pair
    # containing the cursor, else the next pair to the right.
    line = buf.get_line(p.row)
...
#                             | action )
#
# feed() consumes one key and returns ("more" | "cmd" | "abort", Command?).
# This incremental shape is what lets a real frontend drive it key-by-key.
# ──────────────────────────────────────────────────────────────────────────
...
class Command:
    ...
    motion_arg: Optional[str] = None
    tobj: Optional[tuple] = None          # (around: bool, obj: str)
...
class Parser:
    def __init__(self):
        self.visual = False   # in visual mode the grammar inverts: the range
        self.reset()          # already exists, so an operator key *terminates*
    ...
    def reset(self):
        ...
        self.wait: Optional[tuple] = None   # ("reg",) ("char",key) ("obj",ia) ("g",) ("achar",key)

    # -- helpers ---------------------------------------------------------
    def _cmd(self, **kw) -> Command:
        c = Command(register=self.register,
...
SYNONYMS = {
    "x": ("d", "l", False), "X": ("d", "h", False),
    "D": ("d", "$", False), "C": ("c", "$", False),
    "s": ("c", "l", False),
    "S": ("c", None, True), "Y": ("y", None, True),
}


class Engine:
    def __init__(self, buf: Buffer, text: Optional[str] = None):
        self.buf: Buffer = buf if text is None else ListBuffer(text)
        self.cursor = Pos(0, 0)
        self.mode = Mode.NORMAL
        self.curswant = 0
        self.regs = Registers()
        self.last_ft: Optional[tuple] = None       # (key, char) for ; ,
        self.visual_anchor: Optional[Pos] = None
        self.parser = Parser()
        self.status = ""
    ...
    # Operators work on a Span exactly as in normal mode -- selections and
    # operator-pending ranges unify.  (Not dot-recorded; vim approximates
    # visual `.` by reapplying to a same-sized region.)
    ...
    def _exec(self, cmd: Command) -> bool:
        ...
        if cmd.action in SYNONYMS:
            ...
            cmd = Command(register=cmd.register, count=cmd.count,
                          has_count=cmd.has_count, op=op,
                          motion_key=mkey, doubled=doubled)
        ...
        if cmd.op and cmd.doubled:                       # dd yy cc >> <<
            ...
            return self._apply_op(cmd.op, Span(Kind.LINEWISE, Pos(r, 0), Pos(r2, 0)),
                                  cmd.register)
    ...
    def _eval_motion(self, cmd: Command, op_pending: bool) -> Optional[MotionResult]:
    ...
    # ── operators ───────────────────────────────────────────────────────
    def _apply_op(self, op: str, span: Span, reg: Optional[str]) -> bool:
        if op != "y":
            self._snapshot()
        if op == "y":
        ...
        if op in "><":
            ...
            self.cursor = Pos(span.start.row,
                              _first_nonblank(self.buf, span.start.row))
            return True
        if op == "d":
            pieces, kind = self._extract(span)
        ...
        if op == "c":
            ...
            if span.kind is Kind.LINEWISE:
                ...
                self.cursor = Pos(span.start.row,
                                  min(span.start.col, _llen(self.buf, span.start.row)))
    ...
    def _extract(self, span: Span):
        buf = self.buf
        if span.kind is Kind.LINEWISE:
            return ([buf.get_line(r) for r in range(span.start.row, span.end.row + 1)],
                    Kind.LINEWISE)
    ...
    def _exec_action(self, cmd: Command) -> bool:
        ...
        if a == "r":
            ...
            self._snapshot()
            buf.set_line(cur.row, line[:cur.col] + cmd.action_arg * n
                         + line[cur.col + n:])
        ...
        if a == "u":
            ...
            if self._undo:
                ...
                self.cursor = _clamp_col(buf, Pos(min(pos.row, buf.line_count() - 1),
                                                  pos.col))
            return True
    ...
    # ── undo plumbing ───────────────────────────────────────────────────
    # One snapshot per change *command*; a c-op's insert phase edits the
    # buffer directly without snapshotting, so `cwfoo<Esc>` is one undo unit,
    # matching vim.  (vim keeps a full undo tree; we keep a linear stack.)
    def _snapshot(self):
