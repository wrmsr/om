...
async def check_install(node: Node, schema: ReplicationSchema) -> None:
    r1 = await install_node(node, schema)
    assert r1.created_node
    assert (await node.node_id()) == r1.node_id

    # idempotent: nothing is created or migrated the second time, and the identity holds
    r2 = await install_node(node, schema)
    ...
    r3 = await install_node(node, schema, capture_trigger_version=CAPTURE_TRIGGER_VERSION + 1)
    ...
    await insert_row(node, td, row)
    assert (await read_shadow(node, td))[row['id']].version == 1

    # and back down, since the other scenarios assume the default version
    await install_node(node, schema)

    # triggers only, on a table that exists: nothing but trigger ops, and none once they are current
    r4 = await install_node(node, schema, no_manage_base_tables=True)
    assert all(not m.ops for m in r4.migrations)


async def check_install_triggers_only(node: Node, schema: ReplicationSchema) -> None:
    """
    ...
    async with node.db.connect() as conn:
        for td in schema.tables:
            for stmt in r.render_create_statements(dc.replace(td, name=node.table_name(td))):
                await qf.exec(conn, stmt)

    td = schema.table('businesses')
    before = _business('before')
    await insert_row(node, td, before)

    await install_node(node, schema, no_manage_base_tables=True)

    # the row which predates the triggers is backfilled, and one which follows them is captured
    after = _business('after')
    await insert_row(node, td, after)
    sh = await read_shadow(node, td)
    assert sh[before['id']].state == ShadowState(version=1, origin=(await node.node_id()), deleted=False)
    assert sh[after['id']].state == ShadowState(version=1, origin=(await node.node_id()), deleted=False)

    await update_row(node, td, after['id'], {'name': 'after again'})
    assert (await read_shadow(node, td))[after['id']].version == 2


async def check_capture_with_kept_columns(node: Node, schema: ReplicationSchema) -> None:
    """
    ...
    async def num_log_entries() -> int:
        async with node.db.connect() as conn:
            return len([e for e in await node.backend.read_log(conn, node.log_table, after=0, limit=100) if e.key == k])

    await insert_row(node, td, note)
    assert (await read_shadow(node, td))[k].version == 1 and await num_log_entries() == 1

    # the table's own trigger has its say - on sqlite, by an update of its own after the one made - and that is no
    # second change
    await update_row(node, td, k, {'text': 'b'})
    assert (await read_rows(node, td))[k]['updated_at'] > long_ago
    assert (await read_shadow(node, td))[k].version == 2 and await num_log_entries() == 2
    ...
    async with node.db.connect() as conn:
        await node.backend.upsert_rows(conn, td, node.table_name(td), [{**note, 'text': 'c', 'updated_at': long_ago}])
    assert (await read_rows(node, td))[k] == {**note, 'text': 'c'}
    assert (await read_shadow(node, td))[k].version == 3 and await num_log_entries() == 3


async def check_capture(node: Node, schema: ReplicationSchema) -> None:
    await install_node(node, schema)
    td = schema.table('businesses')
    nid = (await node.node_id())

    row = _business('a')
    k = row['id']

    await insert_row(node, td, row)
    assert (await read_shadow(node, td))[k].state == ShadowState(version=1, origin=nid, deleted=False)

    await update_row(node, td, k, {'name': 'b'})
    assert (await read_shadow(node, td))[k].state == ShadowState(version=2, origin=nid, deleted=False)
    assert (await read_shadow(node, td))[k].values == {'id': k, 'name': 'b'}

    await delete_row(node, td, k)
    s = (await read_shadow(node, td))[k]
    ...
    await insert_row(node, td, {**row, 'name': 'c'})
    assert (await read_shadow(node, td))[k].state == ShadowState(version=4, origin=nid, deleted=False)
...
async def check_roundtrip(edge: Node, hub: Node, schema: ReplicationSchema) -> None:
    for n in (edge, hub):
    ...
    edge_id = await edge.node_id()
    assert all(s.origin == edge_id and s.version == 1 and not s.deleted for s in hs.values())
    ...
    await update_row(edge, biz, rows[0]['id'], {'name': 'renamed'})
    await delete_row(edge, biz, rows[1]['id'])
    await sync_link_sweep(link)
    got = await read_rows(hub, biz)
    assert got[rows[0]['id']]['name'] == 'renamed'
    assert rows[1]['id'] not in got
    hs = await read_shadow(hub, biz)
    assert hs[rows[0]['id']].state == ShadowState(version=2, origin=(await edge.node_id()), deleted=False)
    assert hs[rows[1]['id']].state == ShadowState(version=2, origin=(await edge.node_id()), deleted=True)
    ...
    for c in cats:
        await insert_row(edge, cat, c)
    await sync_link_sweep(link)
    assert await read_rows(hub, cat) == {c['id']: c for c in cats}
    await update_row(edge, cat, cats[0]['id'], {'tag': 'retagged'})
    await delete_row(edge, cat, cats[1]['id'])
    await sync_link_sweep(link)
    assert await read_rows(hub, cat) == await read_rows(edge, cat) == {
        cats[0]['id']: {**cats[0], 'tag': 'retagged'},
        **{c['id']: c for c in cats[2:]},
    }
    gone = ShadowState(version=2, origin=await edge.node_id(), deleted=True)
    assert (await read_shadow(hub, cat))[cats[1]['id']].state == gone
    ...
    await insert_row(edge, notes, note)
    await sync_link_sweep(link)
    assert await read_rows(hub, notes) == await read_rows(edge, notes) == {note['id']: note}
    seen = [long_ago]
    for i in range(1, 6):
        await update_row(edge, notes, note['id'], {'text': f'n{i}'})
        await sync_link_sweep(link)
        assert await read_rows(hub, notes) == await read_rows(edge, notes)
        seen.append((await read_rows(hub, notes))[note['id']]['updated_at'])
    ...
    rep = await sync_link_sweep(down)
    assert rep.scanned == 0


async def check_fault_between_apply_and_cursor(edge: Node, hub: Node, schema: ReplicationSchema) -> None:
    ...
    with pytest.raises(InjectedFaultError):
        await sync_link_once(link)

    # the rows landed, the cursor did not
    assert await read_rows(hub, biz) == {r['id']: r for r in rows}
    assert (await link.cursors.read('up', 'businesses')).sweeps == 0

    hub_db.fail_when = lambda _: False
    rep = await sync_link_once(link)
    ...
    assert (await link.cursors.read('up', 'businesses')).sweeps == 1


async def check_step_costs(edge: Node, hub: Node, schema: ReplicationSchema) -> None:
    ...
    for num_rows in (3, 40):
        ...
        for r in rows:
            await insert_row(edge, biz, r)
        assert check.not_none((await step()).tail).applied == num_rows
        assert len(hub_db.writes) == len(schema.tables) + 3

    # as do deletes, which are one more
    for r in rows[:5]:
        await delete_row(edge, biz, r['id'])
    await update_row(edge, biz, rows[5]['id'], {'name': 'renamed'})
    rep = await step()
    assert (check.not_none(rep.tail).applied, check.not_none(rep.tail).deleted) == (1, 5)
    assert len(hub_db.writes) == len(schema.tables) + 4

    assert await read_rows(hub, biz) == await read_rows(edge, biz)


async def check_chunked_apply(edge: Node, hub: Node, schema: ReplicationSchema, *, max_statement_params: int) -> None:
    ...
    hub_db.reset_counts()
    assert check.not_none((await sync_link_once(link)).tail).applied == len(rows)
    assert await read_rows(hub, biz) == await read_rows(edge, biz)
    ...
    for r in rows[:17]:
        await delete_row(edge, biz, r['id'])
    hub_db.reset_counts()
    assert check.not_none((await sync_link_once(link)).tail).deleted == 17
    assert await read_rows(hub, biz) == await read_rows(edge, biz) and len(await read_rows(hub, biz)) == 6
    assert len([s for s in hub_db.writes if s.lstrip().lower().startswith('delete')]) == -(-17 // max_statement_params)


async def check_fanout(hub: Node, n: Node, m: Node, schema: ReplicationSchema) -> None:
    """Two edges through one hub: each sees the other's rows with origins intact, and never re-uploads them."""

    for x in (hub, n, m):
        await install_node(x, schema)
    biz = schema.table('businesses')

    a = _business('from n')
    b = _business('from m')
    await insert_row(n, biz, a)
    await insert_row(m, biz, b)
    ...
    for x in (hub, n, m):
        assert await read_rows(x, biz) == {a['id']: a, b['id']: b}
        sh = await read_shadow(x, biz)
        assert sh[a['id']].origin == (await n.node_id()) and sh[b['id']].origin == (await m.node_id())

    # n's upload sweeps only its own rows, so m's row is never sent back up
    rep = await sync_link_sweep(ups[0])
    assert rep.scanned == 1

    # a violated writer rule is caught: the hub somehow holds n's row at a version n never wrote
    async with hub.db.connect() as conn:
        await hub.backend.upsert_shadows(conn, hub.shadow_name(biz), {a['id']: ShadowState(version=9, origin=(await n.node_id()), deleted=False)})  # noqa
    with pytest.raises(ReplicationConflictError):
        await sync_link_sweep(ups[0])


async def check_worker(edge: Node, hub: Node, broken_hub: Node, schema: ReplicationSchema) -> None:
    ...
    for x in (edge, hub, broken_hub):
        await install_node(x, schema)
    biz = schema.table('businesses')
    await insert_row(edge, biz, _business('w'))
    ...
    assert await read_rows(broken_hub, biz) == await read_rows(edge, biz)
...
async def check_log_tail(edge: Node, hub: Node, schema: ReplicationSchema) -> None:
    ...
    await insert_row(edge, sink, extra)
    ...
    t1 = check.not_none(await sync_link_tail(link))
    assert (t1.entries, t1.keys, t1.applied, t1.drained) == (3, 3, 3, False)
    t2 = check.not_none(await sync_link_tail(link))
    assert (t2.entries, t2.keys, t2.applied, t2.drained) == (3, 2, 2, False)
    t3 = check.not_none(await sync_link_tail(link))
    assert (t3.entries, t3.applied, t3.drained) == (0, 0, True)
    assert await read_rows(hub, biz) == {r['id']: r for r in rows}
    assert not await read_rows(hub, sink)
    assert await link.cursors.read_log('up') == t2.seq == 6

    # an update and a delete arrive through the tail alone, and a repeated key is looked up once
    await update_row(edge, biz, rows[0]['id'], {'name': 'again'})
    await update_row(edge, biz, rows[0]['id'], {'name': 'and again'})
    await delete_row(edge, biz, rows[1]['id'])
    t4 = check.not_none(await sync_link_tail(link))
    assert (t4.entries, t4.keys, t4.applied, t4.deleted) == (3, 2, 1, 1)
    got = await read_rows(hub, biz)
    assert got[rows[0]['id']]['name'] == 'and again' and rows[1]['id'] not in got
    gone = ShadowState(version=2, origin=await edge.node_id(), deleted=True)
    assert (await read_shadow(hub, biz))[rows[1]['id']].state == gone
    ...
    await insert_row(edge, biz, _business('stepped'))
    step = await sync_link_once(link)
    ...
    position = await link.cursors.read_log('up')
    await prune_log(edge, keep_s=0, now=later)
    async with edge.db.connect() as conn:
        assert [e.seq for e in await edge.backend.read_log(conn, edge.log_table, after=0, limit=10)] == [position]
    ...
    await insert_row(edge, biz, after_prune)
    t5 = check.not_none(await sync_link_tail(link))
    assert (t5.entries, t5.applied) == (1, 1) and t5.seq == position + 1
    assert after_prune['id'] in await read_rows(hub, biz)
    ...
    async with edge.db.connect() as conn:
        await qf.exec(conn, f'delete from {edge.backend.qname(edge.log_table)}')  # noqa
    after_empty = _business('after empty')
    await insert_row(edge, biz, after_empty)
    t6 = check.not_none(await sync_link_tail(link))
    assert (t6.entries, t6.applied) == (1, 1) and t6.seq == position + 2
    assert after_empty['id'] in await read_rows(hub, biz)
    rows.append(after_empty)
    assert rows[1]['id'] in await read_shadow(edge, biz) and rows[1]['id'] in await read_shadow(hub, biz)
    await sync_link_sweep(link, prune_tombstones_before=later)
    for n in (edge, hub):
        sh = await read_shadow(n, biz)
        assert rows[1]['id'] not in sh and len(sh) == len(rows)  # four originals plus the stepped one


async def check_no_log(edge: Node, hub: Node, schema: ReplicationSchema) -> None:
    ...
    biz = schema.table('businesses')
    row = _business('quiet')
    await insert_row(edge, biz, row)

    link = _link('up', schema, edge, hub)
    assert await sync_link_tail(link) is None
    step = await sync_link_once(link)
    assert step.tail is None
    assert await read_rows(hub, biz) == {row['id']: row}
    assert (await read_shadow(edge, biz))[row['id']].version == 1


async def check_worker_maintenance(edge: Node, hub: Node, schema: ReplicationSchema) -> None:
    ...
    row = _business('m')
    await insert_row(edge, biz, row)
    await delete_row(edge, biz, row['id'])
    ...
    now[0] += 50.
    assert not (await w.run_once()).maintained
    now[0] += 60.
    assert len((await w.run_once()).maintained) == 2

    # a zero retention leaves nothing in the log behind but its newest entry
    async with edge.db.connect() as conn:
        assert len(await edge.backend.read_log(conn, edge.log_table, after=0, limit=10)) == 1


async def check_worker_pacing(edge: Node, hub: Node, schema: ReplicationSchema) -> None:
    """
    ...
    async def run_until(t: float) -> list[tuple[float, LinkSyncReport]]:
    ...
    for _ in range(num_tables + 1):
        for db in (edge_db, hub_db):
            db.reset_counts()
        [(_, rep)] = await run_until(w.next_due() + 1e-6)
        if not rep.tables:
            break
    ...
    for db in (edge_db, hub_db):
        db.reset_counts()
    swept = [r for _, r in await run_until(now[0] + 60.) if r.tables]
    ...
    async with edge.db.connect() as conn:
        await edge.backend.prune_log(conn, edge.log_table, before=datetime.datetime.now(datetime.UTC) + datetime.timedelta(seconds=60))  # noqa
    async with hub.db.connect() as conn:
        for t in (hub.table_name(biz), hub.shadow_name(biz)):
            await qf.exec(conn, f'delete from {hub.backend.qname(t)}')  # noqa
    assert not await read_rows(hub, biz)
    ran = await run_until(now[0] + 60.)
    behind = [(t, r.tables[0]) for t, r in ran if r.tables and r.tables[0].table == 'businesses']
    ...
    await delete_row(edge, biz, rows[0]['id'])
    assert (await read_shadow(edge, biz))[rows[0]['id']].deleted
    for db in (edge_db, hub_db):
        db.reset_counts()
    await run_until(now[0] + 300.)  # time enough for a sweep to get all the way around
    assert rows[0]['id'] not in await read_rows(hub, biz)
    assert all([rows[0]['id'] not in await read_shadow(n, biz) for n in (edge, hub)])
    assert len(await read_shadow(edge, biz)) == len(await read_shadow(hub, biz)) == len(rows) - 1
