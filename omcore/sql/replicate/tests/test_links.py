...
def test_sqlite_to_postgres(harness) -> None:
    ...
    with hs.sqlite().allocate() as e, hs.postgres().allocate() as h:
        check_roundtrip(sqlite_node('edge', e), postgres_node('hub', h), build_schema())


def test_postgres_to_sqlite(harness) -> None:
    hs = harness[HarnessSandboxes]
    with hs.postgres().allocate() as e, hs.sqlite().allocate() as h:
        check_roundtrip(postgres_node('edge', e), sqlite_node('hub', h), build_schema())


def test_sqlite_to_mysql(harness) -> None:
    hs = harness[HarnessSandboxes]
    with hs.sqlite().allocate() as e, hs.mysql().allocate() as h:
        check_roundtrip(sqlite_node('edge', e), mysql_node('hub', h), build_schema())


def test_mysql_to_postgres(harness) -> None:
    hs = harness[HarnessSandboxes]
    with hs.mysql().allocate() as e, hs.postgres().allocate() as h:
        check_roundtrip(mysql_node('edge', e), postgres_node('hub', h), build_schema())
...
def test_fanout(harness) -> None:
    ...
    with contextlib.ExitStack() as es:
        ...
        check_fanout(postgres_node('hub', h), sqlite_node('n', n), sqlite_node('m', m), build_schema())
...
def test_log_tail_sqlite_to_postgres(harness) -> None:
    hs = harness[HarnessSandboxes]
    with hs.sqlite().allocate() as e, hs.postgres().allocate() as h:
        check_log_tail(sqlite_node('edge', e), postgres_node('hub', h), build_schema())


def test_log_tail_postgres_to_sqlite(harness) -> None:
    hs = harness[HarnessSandboxes]
    with hs.postgres().allocate() as e, hs.sqlite().allocate() as h:
        check_log_tail(postgres_node('edge', e), sqlite_node('hub', h), build_schema())


def test_log_tail_mysql_to_sqlite(harness) -> None:
    hs = harness[HarnessSandboxes]
    with hs.mysql().allocate() as e, hs.sqlite().allocate() as h:
        check_log_tail(mysql_node('edge', e), sqlite_node('hub', h), build_schema())


def test_no_log(harness) -> None:
    hs = harness[HarnessSandboxes]
    with hs.sqlite().allocate() as e, hs.postgres().allocate() as h:
        check_no_log(sqlite_node('edge', e, no_log=True), postgres_node('hub', h), build_schema())


def test_worker_maintenance(harness) -> None:
    hs = harness[HarnessSandboxes]
    with hs.sqlite().allocate() as e, hs.postgres().allocate() as h:
        check_worker_maintenance(sqlite_node('edge', e), postgres_node('hub', h), build_schema())
