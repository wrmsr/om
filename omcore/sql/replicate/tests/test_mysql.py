...
def test_install(harness) -> None:
    with harness[HarnessSandboxes].mysql().allocate() as sb:
        lang.sync_await(check_install(mysql_node('a', sb), build_schema()))


def test_install_triggers_only(harness) -> None:
    with harness[HarnessSandboxes].mysql().allocate() as sb:
        lang.sync_await(check_install_triggers_only(mysql_node('a', sb), build_schema()))


def test_capture(harness) -> None:
    with harness[HarnessSandboxes].mysql().allocate() as sb:
        lang.sync_await(check_capture(mysql_node('a', sb), build_schema()))


def test_capture_with_kept_columns(harness) -> None:
    with harness[HarnessSandboxes].mysql().allocate() as sb:
        lang.sync_await(check_capture_with_kept_columns(mysql_node('a', sb), build_schema()))
