import dataclasses as dc
import typing as ta

from ...specs import FieldSpec
from ...specs import FieldType
from ...specs import ReprFn
from ..generation.base import Generation
from ..generation.base import Generator
from ..generation.globals import REPRLIB_RECURSIVE_REPR_GLOBAL
from ..generation.ops import AddMethodOp
from ..generation.ops import OpRef
from ..generation.ops import Ref
from ..generation.registry import register_generator_type
from ..generation.values import SpecVal
from ..processing.base import ProcessingContext
from .fields import InitFields


##


@dc.dataclass(frozen=True)
class _ReprCacheKey:
    has_own_repr: bool


@register_generator_type
class ReprGenerator(Generator):
    cache_version = 2
    cache_schema = (_ReprCacheKey,)

    def cache_key(self, ctx: ProcessingContext) -> _ReprCacheKey:
        return _ReprCacheKey(
            has_own_repr='__repr__' in ctx.cls.__dict__,
        )

    def generate(self, ctx: ProcessingContext) -> Generation | None:
        if not ctx.cs.repr or self.cache_key(ctx).has_own_repr:
            return None

        ifs = ctx[InitFields]
        fs: ta.Sequence[FieldSpec]
        if ctx.cs.terse_repr:
            # If terse repr will match init param order
            fs = [
                *[f for f in ifs.all if not f.kw_only],
                *[f for f in ifs.all if f.kw_only],
            ]
        else:
            # Otherwise default to dc.fields() order
            fs = sorted(ctx.cs.fields, key=lambda f: f.repr_priority or 0)

        orm = {}
        drf: OpRef | None = None
        if ctx.cs.default_repr_fn is not None:
            drf = OpRef('repr.default_fn')
            orm[drf] = SpecVal(('default_repr_fn',))

        ors: set[Ref] = {REPRLIB_RECURSIVE_REPR_GLOBAL}
        part_lines: list[str] = []
        fnr_g = OpRef.numbered(len(fs))
        for i, f in enumerate(fs):
            if not (f.field_type is FieldType.INSTANCE and f.repr):
                continue

            fn: OpRef[ReprFn] | None = drf
            if f.repr_fn is not None:
                fn = fnr_g('repr.fns.{i}.fn', i)
                orm[fn] = SpecVal(('fields', ctx.cs.field_indexes_by_name[f.name], 'repr_fn'))

            pfx = ''
            if not (ctx.cs.terse_repr and f not in ifs.kw_only):
                pfx = f'{f.name}='

            if fn is not None:
                ors.add(fn)
                part_lines.extend([
                    f'    if (s := {fn.ident()}(self.{f.name})) is not None:',
                    f'        parts.append(f"{pfx}{{s}}")',
                ])
            else:
                part_lines.append(
                    f'    parts.append(f"{pfx}{{self.{f.name}!r}}")',
                )

        body_lines: list[str] = [
            f'@{REPRLIB_RECURSIVE_REPR_GLOBAL.ident}()',
            f'def __repr__(self):',
        ]

        name_src = f'f"{{self.__class__.__qualname__}}{'@{id(self):x}' if ctx.cs.repr_id else ''}'

        if part_lines:
            body_lines.extend([
                f'    parts = []',
                *part_lines,
                f'    return (',
                f'        {name_src}("',
                f'        f"{{\', \'.join(parts)}}"',
                f'        f")"',
                f'    )',
            ])
        else:
            body_lines.append(
                f'    return {name_src}()"',
            )

        return Generation([
            AddMethodOp(
                '__repr__',
                '\n'.join(body_lines),
                frozenset(ors),
            ),
        ], bindings=orm)
