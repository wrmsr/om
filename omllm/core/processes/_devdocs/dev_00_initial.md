...
## Goal
Land phase 1: a tested, injector-bindable `AsyncioProcessManager` under `omllm/core/processes` per `design.md`.
...
- `types/`: `ProcessSpec`/`ProcessStdio` (channels as literals or int fds; `env=None` = inherit, `{}` = clean),
  `ProcessOption` TypedValues (`TerminationPolicy`, `SpoolPolicy`, `SessionMode`, `Credentials`, `Umask`, `Rlimit`,
...
- `spool/`: framed stream (`frames.py`), memory-suffix + spill-prefix storage (`storage.py`), `OutputSpool` with
  ...
  `read(wait=)` semantics chosen: *collect for the window*, returning early only on end-of-output or `max_bytes` — the
  codex `yield_time_ms` shape, batches naturally for a polling model.
- Gotcha: `processes/types` shadows stdlib `types` **only** if you run python with cwd = the processes dir
  (sys.path[0]). Same precedent as `omllm/llm/types`. Always run from repo root.
- Gotcha: forgot `processes/__init__.py` at first -> "relative import beyond top-level package".
