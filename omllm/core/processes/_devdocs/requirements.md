...
- Policies as `TypedValues` families (`ProcessOption`).
- Stuck processes (ignore SIGKILL past hard timeout): abandon + log + event by default; `on_stuck='raise'` option.
- Location: `omllm/core/processes`. Tool-call scope goes on `ToolContext` for now (turn/tool DI scopes come later).
