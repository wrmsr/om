...
`pytest omllm/core/processes` 30 passed / 1 skipped (the 4 new pty tests + everything prior).
...
resource paths"), renamed `omllm/core/processes/_spawn/` -> `omllm/core/processes/spawn/` and updated
...
- `handles.py`: `ProcessPty` role (`has_pty`, `resize`, `get_winsize`) mixed into `Process`.
  ...
  non-pty processes and `ProcessNotAliveError` once the master fd is torn down; the master control fd is closed in
  `_force_close_output`.
