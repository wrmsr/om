...
all operating on the session process scope (`ToolEnvironment.procs`):
...
- Background processes live in `ctx.env.procs` (= `manager.root`, the session scope) - no per-turn/per-tool child
