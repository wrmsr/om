"""
A little end-to-end tour of the procs manager. Run it with:

    ./python -m omllm.core.procs.tests.demos.basic
...
async def _a_main() -> None:
    async with procs.AsyncioProcessManager() as m:
        ...
        stubborn = await m.root.spawn(
            procs.ProcessSpec(['sh', '-c', 'trap "" TERM; echo ready; while :; do sleep 0.1; done']),
            procs.TerminationPolicy(grace_s=0.3),
        )
