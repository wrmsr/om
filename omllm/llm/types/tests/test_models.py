...
from ..models import fill_estimated_token_cost
...
def test_estimate_plain():
    cost = _PRICING.estimate(TokenUsage(input=100, output=10))
...
def test_estimate_cache_write():
    ...
    cost = _PRICING.estimate(TokenUsage(input=8869, output=4, cache_write=8867))
...
def test_estimate_cache_read():
    ...
    cost = _PRICING.estimate(TokenUsage(input=8869, output=4, cache_read=8867))
...
def test_estimate_reasoning_priced_distinctly():
    ...
    cost = pricing.estimate(TokenUsage(input=100, output=50, reasoning=30))
...
def test_estimate_unpriced_cache_bills_as_input():
    pricing = TokenPricing(input=1., output=4.)
    cost = pricing.estimate(TokenUsage(input=100, output=10, cache_read=60))
...
def test_estimate_unknown_components():
    cost = TokenPricing(output=1.).estimate(TokenUsage(input=100, output=10))

    assert cost.input is None
    assert cost.output == pytest.approx(1. * 10 / 1e6)
    assert cost.total == pytest.approx(1. * 10 / 1e6)

    empty = TokenPricing().estimate(TokenUsage(input=100, output=10))
